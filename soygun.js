...
const mekanOlaylari = {};
const kategoriler = ['kucuk', 'yeralti', 'anti', 'merkez', 'hava', 'vip', 'orta', 'sanayi', 'sanat', 'muze', 'banka', 'magaza', 'kamu', 'luks', 'kumarhane', 'depo', 'avm', 'tekno', 'gizli', 'belediye'];

// Güvenli Dosya Okuma (Dosya yoksa veya bozuksa botu çöktürmez, boş dizi atar)
kategoriler.forEach(kategori => {
    const dosyaYolu = path.join(__dirname, 'olaylar', `${kategori}.json`);
    try {
        if (fs.existsSync(dosyaYolu)) {
            const data = fs.readFileSync(dosyaYolu, 'utf8');
            mekanOlaylari[kategori] = JSON.parse(data);
        } else {
            mekanOlaylari[kategori] = [];
        }
    } catch (e) {
        console.error(`⚠️ Olay dosyası okunurken hata oluştu (${kategori}.json):`, e.message);
        mekanOlaylari[kategori] = [];
    }
});
...
let soygunZamanAsimi = null;

// Kilidin sonsuza kadar kalmasını önleyen genel güvenlik fonksiyonu
function kilidiSifirla() {
    aktifSoygunYapan = null;
    if (soygunZamanAsimi) {
        clearTimeout(soygunZamanAsimi);
        soygunZamanAsimi = null;
    }
}
...
function setupSoygun(client, puanlar, savePuanlar) {
    client.on('interactionCreate', async interaction => {
        ...
        // Soygun toplamda 4 dakika içinde bitirilmezse kilidi otomatik aç (Kilitlenme Bug Çözümü)
        soygunZamanAsimi = setTimeout(() => {
            if (aktifSoygunYapan === interaction.user.id) {
                kilidiSifirla();
            }
        }, 4 * 60 * 1000);
        ...
        menuCollector.on('collect', async menuInteraction => {
            ...
            if (tamamlananSoygunlar[interaction.user.id].has(secilenMekanId)) {
                ...
                return await menuInteraction.editReply({
                    content: `🛑 **Zaten burayı patlattınız dostum!** "${mekanBilgi ? mekanBilgi.isim : 'Bu'}" mekanını daha önce başarıyla soydunuz, güvenlik sistemi artık çok sıkı ve kasada atacak ekmek kalmadı. Başka bir hedef seçin! 🎭🏦`,
                    components: []
                });
            }
            ...
            if (!mekan) {
                kilidiSifirla();
                return await menuInteraction.editReply({ content: "⚠️ Geçersiz mekan seçimi!", components: [] });
            }

            let adim = 1;
            const toplamAdim = mekan.adimSayisi;

            const kategoriHavuzu = mekanOlaylari[mekan.kategori] && mekanOlaylari[mekan.kategori].length > 0 
                ? mekanOlaylari[mekan.kategori] 
                : (mekanOlaylari['orta'] && mekanOlaylari['orta'].length > 0 ? mekanOlaylari['orta'] : [{ metin: "Güvenlik kapısını geçmeye çalışıyorsun...", dogru: "Şifreyi gir", yanlis: "Tekme at" }]);
            ...
            for (let j = 0; j < secilenOlaylar.length; j++) {
                ...
                oyunOlaylari.push({
                    metin: temelOlay.metin || "Güvenlik kameralarına yakalanmak üzeresin!",
                    btn1: secenek1 || "Seçenek 1",
                    btn2: secenek2 || "Seçenek 2",
                    dogruCevap: dogruTus
                });
            }
            ...
            const buttonRow = new ActionRowBuilder().addComponents(
                new ButtonBuilder().setCustomId('btn1').setLabel(aktifOlay.btn1.substring(0, 80)).setStyle(ButtonStyle.Danger),
                new ButtonBuilder().setCustomId('btn2').setLabel(aktifOlay.btn2.substring(0, 80)).setStyle(ButtonStyle.Danger)
            );
            ...
            async function sonrakiAdimiIslet(gecerliInteraction, mevcutAdim) {
                ...
                const yeniRow = new ActionRowBuilder().addComponents(
                    new ButtonBuilder().setCustomId('btn1').setLabel(yeniOlay.btn1.substring(0, 80)).setStyle(ButtonStyle.Danger),
                    new ButtonBuilder().setCustomId('btn2').setLabel(yeniOlay.btn2.substring(0, 80)).setStyle(ButtonStyle.Danger)
                );
            ...
            function kurCollector(mesajObjesi, adimNumarasi) {
                ...
                collector.on('collect', async i => {
                    ...
                    const disabledRow = new ActionRowBuilder().addComponents(
                        new ButtonBuilder().setCustomId('btn1').setLabel(olay.btn1.substring(0, 80)).setStyle(ButtonStyle.Secondary).setDisabled(true),
                        new ButtonBuilder().setCustomId('btn2').setLabel(olay.btn2.substring(0, 80)).setStyle(ButtonStyle.Secondary).setDisabled(true)
                    );
